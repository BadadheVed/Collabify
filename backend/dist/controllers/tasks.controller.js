...
exports.ChangeStatus = exports.deleteTask = exports.updateTask = exports.getTasksForTeam = exports.createAndAssignTask = void 0;
...
const createAndAssignTask = async (req, res) => {
    try {
        ...
        const isAdminOrManager = await db_1.db.teamMember.findFirst({
            where: {
                userId,
                teamId,
                role: {
        ...
        const targetMember = await db_1.db.teamMember.findFirst({
        ...
        (0, notifications_service_1.TaskAssigned)(userId, task.title);
        res.status(201).json({
            success: true,
...
const updateTask = async (req, res) => {
    try {
        ...
        const assignedUserTeam = task.assignedTo.teams.find((teamMember) => task.project.teams.some((projectTeam) => projectTeam.id === teamMember.teamId));
        if (!assignedUserTeam) {
            res.status(400).json({
                success: false,
        ...
        if (assignedToId) {
            const isAssignedUserInSameTeam = await db_1.db.teamMember.findFirst({
                where: {
                    userId: assignedToId,
                    teamId: assignedUserTeam.teamId,
                },
...
const deleteTask = async (req, res) => {
    try {
        ...
        await db_1.db.task.delete({ where: { id: taskId } });
        res.status(200).json({
            success: true,
